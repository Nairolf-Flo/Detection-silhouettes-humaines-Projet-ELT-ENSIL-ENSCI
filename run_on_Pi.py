...
repertoire_Humain_Val_Windows = os.path.join(dataDir, 'personalCocoPI','val','nonhumain')
...
for c in ['humain','nonhumain']:
    ...
    for image in toutes_les_images: # parcourt les noms des images du repertoire
        im = np.array(Image.open(os.path.join(repertoire_Val_cat,image)),dtype=np.float32)
        img = im[np.newaxis,:]
        img = np.resize(img,input_shape)
